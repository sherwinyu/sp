...
describe "HumonNodeSuite", ->
  ...
  describe "HumonNode", ->
    ...
    describe "get via unknownProperty", ->
      beforeEach ->
        @spynode_up = sinon.spy node, 'unknownProperty'
        @spynodec_up = sinon.spy nodec, 'unknownProperty'
        @spynodec2_up = sinon.spy nodec2, 'unknownProperty'

      afterEach ->
        @spynode_up.restore()
        @spynodec_up.restore()
        @spynodec2_up.restore()

      it "should call unknownProperty down the chain", ->
        node.get('c.2.nested')
        expect(@spynode_up).toHaveBeenCalledOnce()
        expect(@spynode_up).toHaveBeenCalledWith 'c'#.2.nested'
        # expect(@spynodec_up).toHaveBeenCalledOnce()
        # expect(@spynodec_up).toHaveBeenCalledWith '2.nested'
        # expect(@spynodec2_up).toHaveBeenCalledOnce()
        # expect(@spynodec2_up).toHaveBeenCalledWith 'nested'

      it "should get pathed values on lists", ->
        expect(nodec.get '2.nested').toBe true

      it "should get direct values on lists", ->
        expect(nodec.get '0').toBe false
        expect(nodec.get '1').toBe 'lalala'
        expect(nodec.get '2').toEqual {nested: true}

      it "should get pathed values on hashes", ->
        expect(node.get 'c.2').toEqual {nested: true}
        expect(node.get 'c.2.nested').toBe true

      it "should get direct values on hashes", ->
        expect(node.get 'a').toBe 1
        expect(node.get 'b').toBe 2
        expect(node.get 'c').toEqual [false, 'lalala', {nested: true}]
        expect(nodec2.get 'nested').toBe true

      it "should throw error on literals", ->
