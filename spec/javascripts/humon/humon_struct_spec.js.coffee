...
describe "xHumon", ->
  ...
  ######### SAMPLE ###########
    ...
    describe "computed properties", ->
      describe "when the new prototype lacks them", ->
        beforeEach ->
          Sysys.OtherClass = Ember.Object.extend
            newNodeValue: 'humon node without get node'
            newFunction: -> 'I have a new function'
        it "get 'property' returns undefined", ->
          expect(node.get 'isHash').toBe true
          node.__proto__ = Sysys.OtherClass.prototype
          Sysys.OtherClass.create()
          expect(node.get 'isHash').not.toBeDefined()
