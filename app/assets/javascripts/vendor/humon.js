...
require.define("/lib/parser.js",function(require,module,exports,__dirname,__filename,process,global){/* Jison generated parser */
...
require.define("/src/humon_lexer.coffee",function(require,module,exports,__dirname,__filename,process,global){(function() {
  var BOOL, CALLABLE, CODE, COFFEE_ALIASES, COFFEE_ALIAS_MAP, COFFEE_KEYWORDS, COMMENT, COMPARE, COMPOUND_ASSIGN, HEREDOC, HEREDOC_ILLEGAL, HEREDOC_INDENT, HEREGEX, HEREGEX_OMIT, IDENTIFIER, INDEXABLE, INVERSES, JSTOKEN, JS_FORBIDDEN, JS_KEYWORDS, LINE_BREAK, LINE_CONTINUER, LOGIC, Lexer, MATH, MULTILINER, MULTI_DENT, NOT_REGEX, NOT_SPACED_REGEX, NUMBER, OPERATOR, REGEX, RELATION, RESERVED, Rewriter, SHIFT, SIMPLESTR, STRICT_PROSCRIBED, TRAILING_SPACES, UNARY, WHITESPACE, compact, count, d, key, last, starts, _ref, _ref1,
    __slice = [].slice,
    __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; };

  _ref = require('./humon_rewriter'), Rewriter = _ref.Rewriter, INVERSES = _ref.INVERSES;
  ...
  exports.Lexer = Lexer = (function() {
    var EXPLICIT_STR, IMPLICIT_STR, IMPLICIT_STR_STRICT, OBJ_KEY, SYMBOL_LITERAL;
    ...
    Lexer.prototype.tokenize = function(code, opts) {
      ...
      while (this.chunk = code.slice(i)) {
        ...
        d("");
        d("########i: " + i + " ########");
        d("#### chunk:|" + this.chunk.slice(0, 6) + "|");
        ret = this.whitespaceToken() || this.symbolLiteralToken() || this.lineToken() || this.explicitStringToken() || this.numberToken() || this.implicitStringToken();
        d("## " + (last(this.tokens)));
        if (ret === 0) {
          break;
        }
    ...
    Lexer.prototype.keyToken = function() {
      var match;
      if (!(match = OBJ_KEY.exec(this.chunk))) {
        return 0;
      }
      d("#### obj_key:: |" + match[0]);
      this.token('KEY', match[0]);
      return match[0].length;
    };

    OBJ_KEY = /^([\w ])+\s*:/;

    Lexer.prototype.explicitStringToken = function() {
      var match, string;
      if (!(match = EXPLICIT_STR.exec(this.chunk))) {
        return 0;
      }
      d("#### match::|" + match[1] + "|");
      this.token('STRING', (string = match[1]));
      last(this.tokens).implicit = false;
      return string.length + 2;
    };

    Lexer.prototype.implicitStringToken = function() {
      var match, string;
      if (!(match = IMPLICIT_STR_STRICT.exec(this.chunk))) {
        return 0;
      }
      string = match[0];
      d("#### implicit str::|" + string + "|");
      this.token('STRING', "" + string);
      last(this.tokens).implicit = true;
      return string.length;
    };

    Lexer.prototype.symbolLiteralToken = function() {
      var match, string;
      if (!(match = SYMBOL_LITERAL.exec(this.chunk))) {
        return 0;
      }
      string = match[0];
...
require.define("/src/humon_rewriter.coffee",function(require,module,exports,__dirname,__filename,process,global){(function() {
  ...
  exports.Rewriter = (function() {
    ...
    Rewriter.prototype.sherwinAddImplicitBraces = function() {};
